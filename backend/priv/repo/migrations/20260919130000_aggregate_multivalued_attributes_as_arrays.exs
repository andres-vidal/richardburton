defmodule RichardBurton.Repo.Migrations.AggregateMultivaluedAttributesAsArrays do
  ...
  They are arrays now, as `sources` on this same view already was. The
  ...
  # measures likeness on — the three things the shape decides. `sources` is a
  # list either way, so it is not one of them.
  defp restack(select, words, codes, translators, indexed) do
    ...
    create_operator_indexes(indexed)
    create_search_stack(words, codes)
  end

  # One index per column an operator names, written exactly as
  # `Publication.Index.Query` writes the predicate, or the planner will not use
  # it. `sources` is a list in either shape; the other four follow `indexed`.
  defp create_operator_indexes(indexed) do
    ...
    for field <- ~w(authors original_authors publishers countries) do
      execute("""
      CREATE INDEX flat_publications_#{field}_search_index ON flat_publications
      USING gin (to_tsvector('rb_search', #{indexed.(field)}))
      """)
    end
