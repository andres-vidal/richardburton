defmodule RichardBurton.Publication.IndexTest do
  ...
  describe "all_order/0, search_order/1 and details/2" do
    ...
    test "a page carries an excerpt of each field that answered a row" do
      ...
      assert row.excerpts.sources =~ "[[Berkeley]]"
  ...
  describe "details/2 excerpts" do
    ...
    test "a spelled-out term marks what Postgres read it as" do
      ...
      assert Enum.any?(rows, &(&1[:sources] =~ "[[Berkeley]]"))
    end
  end
  ...
  describe "search/1 with field operators" do
    ...
    # Whether any of a list column's values contains the given text.
    defp holds?(values, text), do: Enum.any?(values, &(String.downcase(&1) =~ text))
    ...
    test "translator and author match different columns" do
      ...
      assert Enum.all?(translators, &holds?(&1.authors, "barrett"))

      refute Enum.empty?(authors)
      assert Enum.all?(authors, &holds?(&1.original_authors, "verissimo"))
    end

    test "publisher and country can be filtered" do
      assert Enum.all?(
               found("publisher:macmillan"),
               &holds?(&1.publishers, "macmillan")
             )

      assert Enum.all?(found("country:GB"), &("GB" in &1.countries))
    ...
    test "a leading minus negates the filter" do
      ...
      refute Enum.any?(without, &("US" in &1.countries))
    end
    ...
    test "author matches the original author, translator the translator" do
      ...
      assert Enum.all?(writers, &holds?(&1.original_authors, "verissimo"))

      refute Enum.empty?(renderers)
      assert Enum.all?(renderers, &holds?(&1.authors, "barrett"))
    end
    ...
    test "an operator applies to its own alternative only" do
      ...
      assert Enum.all?(either, &("BR" in &1.countries or "CA" in &1.countries))
      assert Enum.any?(either, &("BR" in &1.countries))
      assert Enum.any?(either, &("CA" in &1.countries))
    end
    ...
    test "an incomplete operator value matches by prefix" do
      assert Enum.all?(found("translator:barr"), &holds?(&1.authors, "barr"))
