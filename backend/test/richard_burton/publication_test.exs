defmodule RichardBurton.PublicationTest do
  ...
  describe "insert_all/1" do
    ...
    test "when many valid publications are provided, inserts them" do
      ...
      # `all/0` does not order, so compare the sets rather than the orders the
      # database happened to return them in.
      assert Enum.sort_by(Publication.preload(publications), & &1.id) ==
               Enum.sort_by(Publication.all(), & &1.id)
    end
