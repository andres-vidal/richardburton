...
const Entry: FC<{
  ...
  useEffect(() => setValue(name.name), [name.name]);
  ...
  return (
    <li className="flex gap-3 items-center py-2 px-3 rounded border border-gray-200">
      <span className="grow">
        <TextInput
          bordered
          value={value}
 ...
 *
 * Sorted by name, so the spellings of one thing sit together.
 */
const VocabularyList: FC<{
  ...
  async function handleRename(name: Name, to: string) {
    const answer = await write(kind, name.id, to);
    ...
    <div className="space-y-4 max-w-3xl">
      ...
      {names === null ? (
        ...
        <ul aria-label={t(kind)} className="space-y-1">
          {shown.map((name) => (
            <Entry key={name.id} name={name} onRename={handleRename} />
          ))}
        </ul>
      )}
    </div>
